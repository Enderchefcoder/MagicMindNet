...
from magicmindnet._native import (
    vision_rgb_patch_from_image_path as _native_rgb_patch_from_image_path,
)
from magicmindnet._native import (
    vision_rgb_patches_from_image_path as _native_rgb_patches_from_image_path,
)
...
def vision_rgb_patch_from_image_path(path: str) -> list[float]:
    """Load an on-disk image file into a normalized 8×8×3 RGB patch (NCHW)."""
    return _native_rgb_patch_from_image_path(path)
...
def vision_rgb_patches_from_image_path(path: str, grid: int = 1) -> list[list[float]]:
    ...
    return _native_rgb_patches_from_image_path(path, grid)
