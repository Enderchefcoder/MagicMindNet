...
def test_quantize_classifier_int4_changes_backbone_weights(tmp_path: Path):
    ...
    before = json.loads(before_path.read_text(encoding="utf-8"))["tensors"]["backbone"]["data"]
    ai.quantize_classifier(clf, "int4")
    after_path = tmp_path / "after.mmn"
    ai.export_classifier(clf, "safetensors", str(after_path))
    after = json.loads(after_path.read_text(encoding="utf-8"))["tensors"]["backbone"]["data"]
    assert before != after
