...
- **Glint-2 exact parity knobs**: `prelude_layers` (unshared blocks before the loop), `coda_layers` (unshared blocks after the loop and before `final_norm`), `max_loops` (table capacity for `loop_embed` / LoopLoRA, must be ≥ `n_loops`; Glint-2 uses `n_loops=8, max_loops=16`), `attention_window` (sliding causal window — queries only attend to the most recent `w` keys; `None` = full causal attention). All default to zero/None (classic Chatbot). IO roundtrips via `bot.save()` / `ai.load()`.
...
| Feature parity Wave 2 | [feature_parity.md](feature_parity.md) | ~~JSON mode / digit grammar / OpenAIServer~~ |
| Feature parity Wave 3+ | [feature_parity.md](feature_parity.md) | tools, speculative decode, continuous batching (planned) |
